...
class UMPC:
    """
    CasADi-backed NMPC (single SQP/QP per solve).
      - Linearize true RK4 dynamics (FR3TaskDynamics.step) around current rollout
      - Build one QP with dynamics equality constraints
      - Track end-effector position
      - Torque & torque-rate regularization
    ...
    def __init__(self, dyn: FR3TaskDynamics, cfg: MPCConfig):
        ...
        self.Rdu = 1e-3
        self.w_dyn = 0.0  # set >0 to relax dynamics as soft constraints (kept hard here)
        # warm-start cache
        self._u_ws = None

    def _rollout(self, x0, theta, u_seq):
        """Roll out true dynamics with RK4; returns states [S+1,nx]."""
        X = np.zeros((self.cfg.S + 1, self.dyn.nx))
        X[0] = x0
        x = x0.copy()
    ...
    def _linearize_along(self, X, U, theta):
        """Return lists of (A_k, B_k, c_k) along the nominal rollout."""
        A_list, B_list, C_list = [], [], []
        for k in range(self.cfg.S):
            A, B, c = _fd_jacobian(self.dyn.step, X[k], U[k], theta)
            A_list.append(A); B_list.append(B); C_list.append(c)
        return A_list, B_list, C_list

    def solve(self, x0: np.ndarray, theta: np.ndarray, d_seq: np.ndarray,
              u_prev: Optional[np.ndarray] = None) -> np.ndarray:
        S, nx, nu = self.cfg.S, self.dyn.nx, self.dyn.nu
        Qp     = self.cfg.Qp
        Rtau   = self.cfg.Rtau
        Qterm  = self.cfg.Qterm
        Rdu    = self.Rdu

        # --- Warm start for U ---
        if u_prev is None:
            if self._u_ws is None:
                U0 = np.zeros((S, nu))
            else:
                U0 = self._u_ws.copy()
        else:
            U0 = np.tile(u_prev, (S, 1))

        # --- Nominal rollout & linearization ---
        X0 = self._rollout(x0, theta, U0)
        A, B, C = self._linearize_along(X0, U0, theta)

        # --- Build QP in CasADi (decision: U stacked) ---
        # Variables
        U = ca.SX.sym('U', nu*S, 1)
        ...
        # Helper to extract slice of U
        def U_k(k):
            return U[k*nu:(k+1)*nu, 0]

        # Dynamics constraints (hard)
        Xk = X0[0]  # start at given x0
        for k in range(S):
            # predicted next via linear model
            x_lin_next = A[k] @ Xk + B[k] @ U_k(k) + C[k]
            # variable X_{k+1}: we don't expose Xk as decision vars; we enforce equality to fixed linear prediction by chaining
            # add equality to tie the chain with next reference state X0[k+1] + deltaX; implement by introducing residual = x_lin_next - X_{k+1_nominal_minus?}
            # Easiest: enforce chain by defining Xk for next step equal to x_lin_next.
            # For QP constraints, we must express g(U)=0. We'll keep Xk as lifted state carried forward deterministically:
            Xk = x_lin_next

            # Cost terms at step k
            p_k = Xk[0:3]                    # predicted position (linearized)
            e_k = p_k - d_seq[k]             # 3-vector
            cost += Qp * ca.dot(e_k, e_k)    # position tracking
            uk = U_k(k)
            cost += Rtau * ca.dot(uk, uk)    # torque effort
            if k > 0:
                duk = U_k(k) - U_k(k-1)
                cost += Rdu * ca.dot(duk, duk)

        # Terminal cost on position
        pT = Xk[0:3]
        eT = pT - d_seq[-1]
        cost += Qterm * ca.dot(eT, eT)

        # No explicit g constraints because we “folded” linear dynamics into Xk progression.
        g = ca.SX.zeros(0, 1)
        ...
        for k in range(S):
            lbx.extend(tau_lo.tolist())
            ubx.extend(tau_hi.tolist())
        lbx = np.array(lbx).reshape(-1, 1)
        ubx = np.array(ubx).reshape(-1, 1)

        # Build and solve QP (quadratic cost, no constraints g)
        nlp = {'x': U, 'f': cost, 'g': g}
        # Use OSQP via QP wrapper in CasADi; or let IPOPT solve the small NLP
        # OSQP route (preferred for speed):
        try:
            # Convert to QP: f = 1/2 U^T H U + f^T U
            # CasADi qpsol can derive H,f from a quadratic SX expression using 'qpsol' with 'osqp'
            qp = ca.qpsol('qp', 'osqp', {'h': ca.hessian(cost, U)[0], 'g': ca.gradient(cost, U), 'a': None})
            # But qpsol expects standard QP form; simpler: just use IPOPT which handles SX objective directly
            raise RuntimeError  # fall through to IPOPT block below for simplicity/robustness
        except Exception:
            ...
            sol = solver(lbx=lbx, ubx=ubx, lbg=[], ubg=[], x0=U0.reshape(-1, 1))
            U_opt = np.array(sol['x']).reshape(S, nu)
...
#         self.max_iters = 15          # gradient iters per solve
#         self.alpha_step = 1e-3       # initial step size
...
def main():
    ...
    T_rollout, Xr, Yr, Zr = dmp_rollout(dmp_model, dt=dyn.cfg.dt, tau=5.0)
    ...
    for t in range(steps):
        ...
        # UKF predict / update
        ukf.predict(u_prev)
        y_meas = mdl.fk_position(x0[6:13])
        ukf.update(y_meas)

        xe_hat = ukf.xe
        x_hat = xe_hat[0:dyn.nx]
        theta_hat = xe_hat[dyn.nx:]

        # MPC solve
        u_cmd = mpc.solve(x_hat, theta_hat, d_seq, u_prev=u_prev)

        # Integrate true dynamics
        x0 = dyn.step(x0, u_cmd, theta_hat)
        u_prev = u_cmd

        # --- Log torques and time ---
        torque_log.append(u_cmd.copy())
    ...
    # === Optional: quick plot of joint torques ===
    plt.figure(figsize=(10,6))
    for j in range(7):
        plt.plot(time_log, torque_log[:, j], label=f'Joint {j+1}')
